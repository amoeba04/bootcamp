...
def train(train_dataloader, test_dataloader, G_AB, G_BA, D_A, D_B, 
    ...
    for epoch in range(args.epochs):
        ...
        save_models(epoch, G_AB, G_BA, D_A, D_B, args)
...
def main(args):
    ...
    # Training
    train(train_dataloader, test_dataloader, G_AB, G_BA, D_A, D_B, criterion_GAN, criterion_cycle, criterion_identity,
          optimizer_G, optimizer_D_A, optimizer_D_B, lr_scheduler_G, lr_scheduler_D_A, lr_scheduler_D_B,
          device, args)

    # Load saved models for evaluation
    if args.eval:
        G_AB.load_state_dict(torch.load(os.path.join(args.output_dir, "G_AB_latest.pth")))
        G_BA.load_state_dict(torch.load(os.path.join(args.output_dir, "G_BA_latest.pth")))
